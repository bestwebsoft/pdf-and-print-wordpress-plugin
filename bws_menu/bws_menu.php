...
if ( ! function_exists( 'bws_add_menu_render' ) ) {
	function bws_add_menu_render() {
		...
		if ( 'bws_panel' == $page || ( ! $is_main_page && '' == $tab ) ) {
			...
			$plugin_category = isset( $_GET['category'] ) ? esc_attr( sanitize_text_field( $_GET['category'] ) ) : 'all';
			...
			<div class="bws-header">
				...
				<div class="bws-nav-tab-wrapper">
					<?php if ( $is_main_page ) { ?>
						...
						?>
						" href="<?php echo esc_url( self_admin_url( 'admin.php?page=bws_panel' ) ); ?>"><?php _e( 'Plugins', 'bestwebsoft' ); ?></a>
						<a class="bws-nav-tab
						<?php
						if ( 'bws_themes' == $page ) {
							echo esc_attr( ' bws-nav-tab-active' );}
						?>
						" href="<?php echo esc_url( self_admin_url( 'admin.php?page=bws_themes' ) ); ?>"><?php _e( 'Themes', 'bestwebsoft' ); ?></a>
						<a class="bws-nav-tab
						<?php
						if ( 'bws_system_status' == $page ) {
							echo esc_attr( ' bws-nav-tab-active' );}
						?>
						" href="<?php echo esc_url( self_admin_url( 'admin.php?page=bws_system_status' ) ); ?>"><?php _e( 'System status', 'bestwebsoft' ); ?></a>
					<?php } else { ?>
						<a class="bws-nav-tab
						<?php
						if ( ! isset( $_GET['tab'] ) ) {
							echo esc_attr( ' bws-nav-tab-active' );}
						?>
						" href="<?php echo esc_url( self_admin_url( 'admin.php?page=' . $page ) ); ?>"><?php _e( 'Plugins', 'bestwebsoft' ); ?></a>
						<a class="bws-nav-tab
						<?php
						if ( 'themes' == $tab ) {
							echo esc_attr( ' bws-nav-tab-active' );}
						?>
						" href="<?php echo esc_url( self_admin_url( 'admin.php?page=' . $page . '&tab=themes' ) ); ?>"><?php _e( 'Themes', 'bestwebsoft' ); ?></a>
						<a class="bws-nav-tab
						<?php
						if ( 'system-status' == $tab ) {
							echo esc_attr( ' bws-nav-tab-active' );}
						?>
						" href="<?php echo esc_url( self_admin_url( 'admin.php?page=' . $page . '&tab=system-status' ) ); ?>"><?php _e( 'System status', 'bestwebsoft' ); ?></a>
					<?php } ?>
				</div>
						...
						<form class="bws-membership-form" method="post" action="">
							...
							<?php } else { ?>
								<div class="bws_form_input_wrap">
									...
									?>
									 type="text" placeholder="<?php _e( 'Enter your license key', 'bestwebsoft' ); ?>" maxlength="100" name="bws_license_key" value="<?php echo esc_attr( $bws_license_key ); ?>" />
									<div class="bws_error" 
									<?php
									if ( '' == $error ) {
			...
			<div class="bws-wrap-content wrap">
				<?php if ( 'bws_panel' == $page || ( ! isset( $_GET['tab'] ) && ! $is_main_page ) ) { ?>
					...
					<h1>
						...
						<a href="<?php echo esc_url( self_admin_url( 'plugin-install.php?tab=upload' ) ); ?>" class="upload page-title-action add-new-h2"><?php _e( 'Upload Plugin', 'bestwebsoft' ); ?></a>
					</h1>
					<?php
					if ( isset( $_GET['error'] ) ) {
						...
						?>
						<div id="message" class="error is-dismissible"><p><?php echo esc_html( $errmsg ); ?></p></div>
					...
					if ( isset( $_POST['bws_plugin_action_submit'] ) && isset( $_POST['bws_install_plugin'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'bws_license_install_nonce_name' ) ) {
						...
						<ul class="subsubsub">
							<li>
								...
								if ( ! isset( $_GET['sub'] ) ) {
									echo 'class="current" ';}
								?>
								href="<?php echo esc_url( self_admin_url( $category_href ) ); ?>"><?php _e( 'All', 'bestwebsoft' ); ?></a>
							</li> |
							<li>
								<a 
								<?php
								if ( isset( $_GET['sub'] ) && 'installed' == sanitize_text_field( $_GET['sub'] ) ) {
									echo 'class="current" ';}
								?>
								href="<?php echo esc_url( self_admin_url( $category_href . '&sub=installed' ) ); ?>"><?php _e( 'Installed', 'bestwebsoft' ); ?></a>
							</li> |
							<li>
								<a 
								<?php
								if ( isset( $_GET['sub'] ) && 'not_installed' == sanitize_text_field( $_GET['sub'] ) ) {
									echo 'class="current" ';}
								?>
								href="<?php echo esc_url( self_admin_url( $category_href . '&sub=not_installed' ) ); ?>"><?php _e( 'Not Installed', 'bestwebsoft' ); ?></a>
							</li>
						</ul>
						<div class="clear"></div>
						<div class="bws-filter-top">
							...
							<div class="bws-filter-top-inside">
								...
								<ul class="bws-category">
									<li>
										<?php $sub_in_url = ( isset( $_GET['sub'] ) && in_array( sanitize_text_field( $_GET['sub'] ), array( 'installed', 'not_installed' ) ) ) ? '&sub=' . $_GET['sub'] : ''; ?>
										<a 
										<?php
										if ( 'all' == $plugin_category ) {
											echo ' class="bws-active"';}
										?>
										 href="<?php echo esc_url( self_admin_url( $current_page . $sub_in_url ) ); ?>"><?php _e( 'All', 'bestwebsoft' ); ?>
						...
						<div class="bws-products">
							...
							foreach ( $bws_plugins as $key_plugin => $value_plugin ) {
								...
								if ( ( isset( $_GET['sub'] ) && 'installed' === sanitize_text_field( $_GET['sub'] ) && ! $is_pro_installed && ! $is_installed ) ||
										 ( isset( $_GET['sub'] ) && 'not_installed' === sanitize_text_field( $_GET['sub'] ) && ( $is_pro_installed || $is_installed ) ) ) {
									continue;
								}
									...
									<div class="bws_product_content">
										<div class="bws_product_title"><a <?php echo wp_kses_data( $link_attr ); ?>><?php echo esc_html( $value_plugin['name'] ); ?></a></div>
											...
											<div class="bws_product_description">
												<?php echo ( strlen( $value_plugin['description'] ) > 100 ) ? mb_substr( $value_plugin['description'], 0, 100 ) . '...' : esc_html( $value_plugin['description'] ); ?>
											</div>
